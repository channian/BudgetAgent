...
前置：
  ...
  • 套件：pip install openpyxl xlrd python-pptx PyPDF2 python-docx
          （xlrd 用於舊版 .xls；openpyxl 用於新版 .xlsx/.xlsm）
    ...
    try:
        ...
        if ext == ".xls":
            try:
                ...
                return "[注意：xlrd 未安裝，無法讀取 .xls 檔案，請執行 pip install xlrd]"
    except Exception:
        pass
    return "[注意：此檔案內容提取受限]"
...
def process():
    ...
    for folder in folders:
        ...
        for fname in files:
            ...
            if ext in (".pdf", ".pptx", ".docx", ".txt", ".xlsx", ".xlsm", ".xls"):
